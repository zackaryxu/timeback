...
(async()=>{
  ...
  try{
    ...
    const badge=el('p',c.status==='official'?'Official Chapter':'Upcoming Chapter');badge.className=`cp-status cp-status--${c.status==='official'?'official':'upcoming'}`;
    ...
    else if(c.status==='official'){const note=el('p',`First meeting held ${c.firstGathering.date}`);note.className='cp-meeting-note';wrapper.append(note);}
